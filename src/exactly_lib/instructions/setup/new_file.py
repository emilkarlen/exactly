...
class _Instruction(SetupPhaseInstruction):
    ...
    def value_usages(self) -> list:
        return self.file_info.file_ref.value_references_of_paths()

    def main(self,
             environment: InstructionEnvironmentForPostSdsStep,
             os_services: OsServices,
             settings_builder: SetupSettingsBuilder) -> sh.SuccessOrHardError:
        error_message = new_file_utils.create_file(self.file_info, environment.path_resolving_environment)
