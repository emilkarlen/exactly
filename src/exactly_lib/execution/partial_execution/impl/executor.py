...
class _PartialExecutor:
    def __init__(self,
        ...
        self._act_source_and_executor_constructor = \
            conf.conf_phase_values.act_phase_handling.atc_exe_parser
    ...
    def _act__parse_atc_executor(self) -> Optional[PhaseStepFailure]:
        ...
        atc_executor_parser = self.conf_values.act_phase_handling.atc_exe_parser
    ...
    def _act__validate_symbols(self) -> Optional[PhaseStepFailure]:
        ...
        def action() -> Optional[PhaseStepFailure]:
            ...
            res = executor.apply(self._atc_executor)
            if res is None:
                return None
            else:
    ...
    def _act__validate_pre_sds(self) -> Optional[PhaseStepFailure]:
        ...
        def action() -> Optional[PhaseStepFailure]:
            res = self._atc_executor.validate_pre_sds(self._instruction_environment_pre_sds)
            if res.is_success:
                return None
            else:
                return failure_con.apply(PartialExeResultStatus(res.status.value),
