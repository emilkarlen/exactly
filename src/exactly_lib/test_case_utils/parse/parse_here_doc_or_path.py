...
def parse_from_parse_source(source: ParseSource,
                            conf: RelOptionArgumentConfiguration = CONFIGURATION) -> StringOrPathResolver:
...
def parse_from_token_parser(token_parser: TokenParser,
                            ...
                            consume_last_here_doc_line: bool = True) -> StringOrPathResolver:
    ...
    if path is not None:
        return StringOrPathResolver(SourceType.PATH, None, path)
    else:
        source_type, resolver = parse_string_or_here_doc_from_token_parser(token_parser, consume_last_here_doc_line)
        return StringOrPathResolver(source_type, resolver, None)
