...
class _FileMatcherNameGlobPattern(FileMatcherImplBase):
    ...
    NAME = file_matcher.PATH_MATCHER_NAME
    ...
    def matches_w_trace(self, model: FileMatcherModel) -> MatchingResult:
        tb = self.__tb_with_expected().append_details(
            custom_details.actual(
                custom_details.PathDdvAndPrimitiveDetailsRenderer(model.path.describer)
            )
        )
        path_model = model.path.primitive
        return tb.build_result(path_model.match(self._glob_pattern))
