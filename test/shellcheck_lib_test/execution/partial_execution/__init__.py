...
class Test(unittest.TestCase):
    def test_act_phase_should_be_transformed_to_script_file(self):
        py_unit_test_case.py3_test(
            self,
                _pe_test_case(act_phase_should_be_transformed_to_script_file.TestCaseDocument().test_case),
            act_phase_should_be_transformed_to_script_file.assertions)

    def test_result_from_act_phase_should_be_saved(self):
        py_unit_test_case.py3_test(
            self,
                _pe_test_case(result_from_action_phase_should_be_saved.TestCaseDocument().test_case),
...
def _pe_test_case(test_case: test_case_doc.TestCase) -> sut.TestCase:
    return sut.TestCase(test_case.setup_phase,
                        test_case.act_phase,
