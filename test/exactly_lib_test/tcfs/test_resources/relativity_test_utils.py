...
def hds_2_hds_case_dir(hds: HomeDs) -> pathlib.Path:
    return hds.case_dir


def hds_2_hds_act_dir(hds: HomeDs) -> pathlib.Path:
    return hds.act_dir


def tcds_2_hds_case_dir(tcds: TestCaseDs) -> pathlib.Path:
    return tcds.hds.case_dir


def tcds_2_hds_act_dir(tcds: TestCaseDs) -> pathlib.Path:
    return tcds.hds.act_dir


def sds_2_act_dir(sds: SandboxDs) -> pathlib.Path:
    return sds.act_dir


def sds_2_result_dir(sds: SandboxDs) -> pathlib.Path:
    return sds.result.root_dir


def sds_2_tmp_user_dir(sds: SandboxDs) -> pathlib.Path:
    return sds.user_tmp_dir


def sds_2_cwd_dir(sds: SandboxDs) -> pathlib.Path:
    return pathlib.Path().cwd()


def tcds_2_cwd_dir(tcds: TestCaseDs) -> pathlib.Path:
    return pathlib.Path().cwd()
