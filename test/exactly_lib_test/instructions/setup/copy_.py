...
class TestParse(unittest.TestCase):
    def test_fail_when_there_is_no_arguments(self):
        ...
        with self.assertRaises(SingleInstructionInvalidArgumentException):
            sut.Parser().parse(ARBITRARY_FS_LOCATION_INFO, ParseSource(instruction_argument))
    ...
    def test_succeed_when_there_is_exactly_two_arguments(self):
        ...
        arguments_layout_cases = [
            NameAndValue(
                'single line',
                instruction_argument,
            ),
            NameAndValue(
                'arguments on following line',
                '\n' + instruction_argument,
            ),
        ]
        for arguments_layout_case in arguments_layout_cases:
            with self.subTest(arguments_layout_case=arguments_layout_case.name):
                for source in equivalent_source_variants__with_source_check__consume_last_line(
                        self, arguments_layout_case.value):
                    sut.Parser().parse(ARBITRARY_FS_LOCATION_INFO, source)
