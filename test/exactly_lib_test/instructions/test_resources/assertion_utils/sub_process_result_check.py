...
class IsSuccess(ValueAssertionBase[spe.ResultAndStderr]):
    def _apply(self,
...
class IsFailure(ValueAssertionBase[spe.ResultAndStderr]):
    def _apply(self,
               put: unittest.TestCase,
               value,
               message_builder: asrt.MessageBuilder):
...
class ExitCodeIs(ValueAssertionBase[spe.ResultAndStderr]):
    def __init__(self,
                 exit_code: int):
...
class StderrContentsIs(ValueAssertionBase[spe.ResultAndStderr]):
    def __init__(self,
                 stderr_contents: str):
