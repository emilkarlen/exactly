...
from exactly_lib.value_definition.concrete_values import FileRefValue, StringValue, ValueVisitor, SymbolValue
...
def value_equals3(expected: SymbolValue) -> asrt.ValueAssertion:
    return _EqualsValue(expected)


def file_ref_value_equals(expected: FileRefValue) -> asrt.ValueAssertion:
    return _AssertFileRefIsIdenticalToInAutoGeneratedFakeEnvironment(expected)


def equals_file_ref_value2(expected_relativity_and_paths: FileRef,
...
def equals_string_value2(expected_value: str,
                         symbol_references_assertion: asrt.ValueAssertion,
                         symbol_table: SymbolTable) -> asrt.ValueAssertion:
    return asrt.is_instance_with(StringValue, asrt.and_([
...
def equals_string_value3(expected: StringValue) -> asrt.ValueAssertion:
...
class _FileRefAssertionBase(asrt.ValueAssertion):
    def apply(self,
        ...
        put.assertIsInstance(value, FileRefValue,
                             'Actual value is expected to be a ' + str(FileRefValue))
...
class _AssertFileRefIsIdenticalToInAutoGeneratedFakeEnvironment(_FileRefAssertionBase):
    ...
    def _check_path_suffix(self,
        ...
        path_suffix = self._expected_file_ref(sym_tbl).path_suffix(sym_tbl)
        equals_path_part(path_suffix).apply(put,
                                            actual.path_suffix(sym_tbl),
                                            message_builder)
...
class _AssertStringValueIsIdenticalToInAutoGeneratedFakeEnvironment(asrt.ValueAssertion):
    ...
    def apply(self,
              ...
              value,
              message_builder: asrt.MessageBuilder = asrt.MessageBuilder()):
        put.assertIsInstance(value, StringValue,
                             'Actual value is expected to be a ' + str(StringValue))
...
class _EqualsValueVisitor(ValueVisitor):
    ...
    def _visit_file_ref(self, expected: FileRefValue):
        return file_ref_value_equals(expected).apply(self.put, self.actual, self.message_builder)

    def _visit_string(self, expected: StringValue):
        return equals_string_value3(expected).apply(self.put, self.actual, self.message_builder)


class _EqualsValue(asrt.ValueAssertion):
    def __init__(self, expected: SymbolValue):
