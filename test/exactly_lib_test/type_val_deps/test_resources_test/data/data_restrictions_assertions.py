...
from exactly_lib.type_val_deps.sym_ref.data.reference_restrictions import FailureOfIndirectReference
...
from exactly_lib_test.type_val_deps.test_resources.data import data_restrictions_assertions as sut
...
class TestEqualsReferenceRestrictions(unittest.TestCase):
    def test_pass(self):
        cases = [
            (
                sut.equals_reference_restrictions__convertible_to_string(r.ReferenceRestrictionsOnDirectAndIndirect(
                    vr.AnyDataTypeRestriction())),
                r.ReferenceRestrictionsOnDirectAndIndirect(
                    vr.AnyDataTypeRestriction()),
            ),
            ...
            (
                sut.equals_reference_restrictions__convertible_to_string(r.OrReferenceRestrictions([])),
                r.OrReferenceRestrictions([]),
            ),
        ]
    ...
    def _fail(self, expected: r.DataTypeReferenceRestrictions, actual: ReferenceRestrictions):
        assertion = sut.equals_reference_restrictions__convertible_to_string(expected)
        assert_that_assertion_fails(assertion, actual)
...
class TestIsStringMadeUpOfJustStringsReferenceRestrictions(unittest.TestCase):
    def test_pass(self):
        ...
        assertion = sut.is_reference_restrictions__string_made_up_of_just_strings()
        assertion.apply_without_message(self, actual)
