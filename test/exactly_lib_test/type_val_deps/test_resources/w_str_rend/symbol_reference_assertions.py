...
from exactly_lib_test.type_val_deps.test_resources.w_str_rend.data_restrictions_assertions import \
    is_reference_restrictions__on_direct_and_indirect, equals_reference_restrictions__w_str_rendering
...
def equals_data_type_symbol_reference(expected: SymbolReference) -> Assertion[SymbolReference]:
    ...
    return asrt_sym_ref.matches_reference_2(expected.name,
                                            equals_reference_restrictions__w_str_rendering(restrictions))
...
def is_reference_to__w_str_rendering(symbol_name: str
    ...
    return matches_reference_2(
        symbol_name,
        asrt_data_ref_restriction.is_reference_restrictions__w_str_rendering(),
    )
...
def is_reference_to_string__w_all_indirect_refs_are_strings(symbol_name: str) -> Assertion[SymbolReference]:
    return matches_reference_2(
        symbol_name,
        asrt_data_ref_restriction.is_reference_restrictions__string__w_all_indirect_refs_are_strings()
    )
