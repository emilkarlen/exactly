...
class TestCaseBase(unittest.TestCase):
    def _check_selector(self,
                        selector_to_check: sut.DirContentsSelector,
                        dir_to_select_from: pathlib.Path,
                        expectation: Expectation):
        # ACT #

        actual = selector_to_check.files(dir_to_select_from)

        # ASSERT #

        expected = [dir_to_select_from / f.name for f in expectation.file_system_elements]
        ...
        expectation.description.apply_with_message(self, selector_to_check.restriction_descriptions,
                                                   'descriptions')
...
class TestNamePattern(TestCaseBase):
    ...
    def test(self):
        with tmp_dir(self.full_dir_contents) as dir_to_select_from:
            selector = sut.NamePatternSelector(self.pattern)
            self._check_selector(
                selector,
                dir_to_select_from,
                Expectation(
    ...
    def test(self):
        with tmp_dir(self.full_dir_contents) as dir_to_select_from:
            for file_type, expected_file_system_elements in self.file_type_cases:
                selector = sut.FileTypeSelector(file_type)
...
class TestAnd(TestCaseBase):
    ...
    def test_WHEN_no_selectors_THEN_all_files_should_be_selected(self):
        with tmp_dir(self.full_dir_contents) as dir_to_select_from:
            selector = sut.AndSelector([])
            self._check_selector(
                selector,
                dir_to_select_from,
                Expectation(
    ...
    def test_WHEN_one_selector_THEN_selection_SHOULD_be_identical_to_selection_of_the_selector(self):
        with tmp_dir(self.full_dir_contents) as dir_to_select_from:
            selector = sut.AndSelector([sut.FileTypeSelector(file_properties.FileType.REGULAR)])
            self._check_selector(
                selector,
                dir_to_select_from,
                Expectation(
                    file_system_elements=self.regular_files,
                    description=DESCRIPTION_IS_SINGLE_STR,
                )
            )

    def test_WHEN_multiple_selectors_THEN_selection_SHOULD_be_intersection_of_all_selections(self):
        with tmp_dir(self.full_dir_contents) as dir_to_select_from:
            selector = sut.AndSelector([
                sut.FileTypeSelector(file_properties.FileType.REGULAR),
                sut.NamePatternSelector(self.sym_link_name_pattern)])
            self._check_selector(
                selector,
                dir_to_select_from,
                Expectation(
                    file_system_elements=[self.sym_link_to_regular_file],
                    description=asrt.matches_sequence([asrt.is_instance(str),
