...
OLD_EXECUTABLE_FILE_FOR_PY_SRC_AS_ONE_AND_ONLY_ARG = ExecutableFileWithArgsResolver(
...
EXECUTABLE_FILE_FOR_PY_SRC_AS_ONE_AND_ONLY_ARG = new_command_resolvers.for_executable_file(
    file_ref_resolvers2.constant(file_refs.absolute_file_name(sys.executable))
).new_with_additional_arguments(list_resolvers.from_str_constants([PY_ARG_FOR_EXECUTING_SOURCE_ON_COMMAND_LINE]))


def old_command_resolver_for_source_on_command_line(python_source: str) -> CommandResolverForExecutableFile:
    return CommandResolverForExecutableFile(OLD_EXECUTABLE_FILE_FOR_PY_SRC_AS_ONE_AND_ONLY_ARG,
                                            list_resolvers.from_str_constants([python_source]))


def command_resolver_for_source_on_command_line(python_source: str) -> NewCommandResolver:
