...
class TestWithTransformation(unittest.TestCase):
    ...
    def test_references_of_transformer_and_matcher_sdv_SHOULD_be_reported(self):
        # ARRANGE #
        ...
        sdv = sut.new_with_transformation(
            StringTransformerSdvReference(trans_ref_info.name),
            sut.new_reference(matcher_ref_info.name, ExpectationType.POSITIVE),
        )
        # ACT #

        actual = sdv.references
