...
from exactly_lib.impls.types.string_model.command_output import string_model as sut
    ...
    def runTest(self):
        ...
        for proc_output_file in ProcOutputFile:
            ...
            with self.subTest(output_channel=proc_output_file):
                model_constructor = _ModelConstructorForPySourceProgramViaCmdLine(
                    python_source,
                    ModelMaker(
        ...
        for proc_output_file in ProcOutputFile:
            for ignore_exit_code in [False, True]:
                with self.subTest(ignore_exit_code=ignore_exit_code,
                    ...
                    # ACT & ASSERT #
                    model_constructor = _ModelConstructorForExeFile(
                    ...
                    checker = model_checker.Checker(
                        self,
                        model_constructor,
                        model_checker.Expectation.hard_error(
                            may_depend_on_external_resources=asrt.equals(True),
                        ),
                    )
                    # ACT & ASSERT #
...
class TestNonZeroExitCode(unittest.TestCase):
    ...
    def _check_exit_codes(self,
                          ...
                          expected_primitive: Callable[[str], model_checker.Expectation],
                          ):
        # ARRANGE #
    ...
    @staticmethod
    def _contents_access_raises_hard_error(contents_on_output_channel: str) -> model_checker.Expectation:
        return model_checker.Expectation.hard_error(
            may_depend_on_external_resources=asrt.equals(True)
        )

    @staticmethod
    def _contents_is_output_from_program(contents_on_output_channel: str) -> model_checker.Expectation:
        return model_checker.Expectation(
...
class _ModelConstructorForPySourceProgramViaCmdLine(model_checker.ModelConstructor):
    def __init__(self,
                 py_source: str,
...
class _ModelConstructorForExeFile(model_checker.ModelConstructor):
    def __init__(self,
                 exe_file: Path,
                 model_maker: ModelMaker,
                 ):
