...
def path_resolver_test_impl(valid_relativities: PathRelativityVariants) -> PathResolver:
    relativity = list(valid_relativities.rel_option_types)[0]
    assert isinstance(relativity, RelOptionType)
    return path_resolvers.constant(path_test_impl('path_test_impl', relativity))
...
class _ValueCorrespondingToValueRestriction(vr.ValueRestrictionVisitor):
    ...
    def visit_path_relativity(self, x: vr.PathRelativityRestriction) -> DataValueResolver:
        return path_resolver_test_impl(x.accepted)


def _resolver_container(value: PathResolver) -> SymbolContainer:
