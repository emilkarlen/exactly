...
from exactly_lib_test.test_case_file_structure.test_resources import string_value as sv
...
class _EqualsSymbolValueResolverBase(asrt.ValueAssertion):
    ...
    def apply(self,
        ...
        self._custom_check_of_resolver(put, actual)
...
class _FileRefResolverAssertionBase(asrt.ValueAssertion):
    def apply(self,
        ...
        self._check_symbol_references(put, message_builder.for_sub_component('symbol_references'),
                                      value.references)

        environment = self._get_environment()

        expected_file_ref = self._expected_file_ref(environment.symbols)

        actual_file_ref = value.resolve(environment.symbols)
...
class _FileRefResolverAssertionBase2(_EqualsSymbolValueResolverBase):
    def __init__(self,
                 expected_symbol_references: asrt.ValueAssertion,
                 expected_resolved_value: asrt.ValueAssertion,
                 symbols_for_checking_resolving: SymbolTable):
    ...
    def _custom_check_of_resolver(self,
        ...
        put.assertIsInstance(actual, FileRefResolver,
                             'Actual value is expected to be a ' + str(FileRefResolver))
        assert isinstance(actual, FileRefResolver)
...
class _AssertFileRefResolverHasSpecifiedProperties(_FileRefResolverAssertionBase):
    ...
    def _file_ref_assertion(self, expected_file_ref: FileRef) -> asrt.ValueAssertion:
        return equals_file_ref(expected_file_ref)
...
class _AssertFileRefResolverIsIdenticalToInAutoGeneratedFakeEnvironment(_FileRefResolverAssertionBase2):
    def __init__(self, expected: FileRefResolver):
        ...
        super().__init__(equals_symbol_references(expected.references),
...
class _AssertStringValueIsIdenticalToInAutoGeneratedFakeEnvironment(asrt.ValueAssertion):
    ...
    def apply(self,
        ...
        self._check_fragments(put, message_builder, value.fragments)
        self._check_symbol_references(put, message_builder.for_sub_component('symbol_references'),
                                      value.references)
        ...
        resolved_value_assertion = sv.equals_string_value(expected_string)
        resolved_value_assertion.apply(put, actual_string, message_builder.for_sub_component('resolved-value'))
    ...
    def _check_fragments(self,
                         ...
                         message_builder: asrt.MessageBuilder,
                         actual):
        assertion = equals_string_fragments(self._expected.fragments)
        assertion.apply(put, actual, message_builder.for_sub_component('fragments'))

    def _check_symbol_references(self,
                                 put: unittest.TestCase,
                                 message_builder: asrt.MessageBuilder,
                                 actual: list):
        assertion = equals_symbol_references(self._expected.references)
