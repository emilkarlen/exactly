...
class TheConfigurationBase(Configuration):
    ...
    def program_that_copes_stdin_to_stdout(self, hds: HomeDirectoryStructure) -> list:
        yield _instructions_for_file_in_home_dir(hds,
...
def _instructions_for_file_in_home_dir(hds: HomeDirectoryStructure, statements: list) -> list:
    with open(str(hds.case_dir / 'sut.py'), 'w') as f:
